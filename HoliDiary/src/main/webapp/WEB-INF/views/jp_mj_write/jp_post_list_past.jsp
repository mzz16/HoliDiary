...
<script type="text/javascript">
	function countPostChange() {
		...
		location.href = "jp_post-list.past?userId=${User.userID}&nowPage=${paging.nowPage}&cntPerPage="
				+ countPost;
	}
	...
	function pastList() {
		var countPost = document.getElementById('cntPerPage').value;
		location.href = "jp_post-list.past?userId=${User.userID}&nowPage=1&cntPerPage=10";
	}

	function recommendList() {
		var countPost = document.getElementById('cntPerPage').value;
		location.href = "jp_post-list.recommend?userId=${User.userID}&nowPage=1&cntPerPage=10";
	}
</script>
...
<body>
	...
	<div style="text-align: center; margin-top: 15px; font-size: 10pt;">
		<c:if test="${paging.startPage != 1 }">
			<a
				href="jp_post-list.past?userId=${User.userID}&nowPage=${paging.startPage - 1}&cntPerPage=${paging.cntPerPage }">&lt;</a>
		</c:if>
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
			...
			<c:choose>
				...
				<c:when test="${p != paging.nowPage }">
					<a
						href="jp_post-list.past?userId=${User.userID}&nowPage=${p }&cntPerPage=${paging.cntPerPage }">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.endPage != paging.lastPage }">
			<a
				href="jp_post-list.past?userId=${User.userID}&nowPage=${paging.endPage + 1}&cntPerPage=${paging.cntPerPage }">&gt;</a>
		</c:if>
	</div>
