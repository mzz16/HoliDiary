...
<script type="text/javascript">
	function countPostChange() {
		...
		location.href="jp_post-Gallery.past?userId=${User.userID}&nowPage=${paging.nowPage}&cntPerPage="+countPost;
	}
...
<body>
	...
	<div id="postListOrder">
		<p>
			...
			<button class="postListOrderA" style="font-weight: 600" onclick="pastList();">過去順</button>	
			<button class="postListOrderA" onclick="recommendList();">いいね順</button>		
		</p>
	</div>
	...
	<c:forEach var="result" items="${DiaryPosts}" varStatus="status">
		...
		<tr>
			...
			<tr>
				<td class="postDate" style="font-size: 9pt; text-align: right;">likes ${result.postRecommend } /</td>
				<td class="postViewCount" style="font-size: 9pt; text-align: left;">views ${result.postView }</td>
			</tr>
		</table>
	</c:forEach>
	</c:if>
	...
	<div style="text-align: center; margin-top: 15px; font-size: 10pt;">
		<c:if test="${paging.startPage != 1 }">
			<a href="jp_post-Gallery.past?userId=${User.userID}&nowPage=${paging.startPage - 1}&cntPerPage=${paging.cntPerPage }">&lt;</a>
		</c:if>
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage }">
					<a href="jp_post-Gallery.past?userId=${User.userID}&nowPage=${p }&cntPerPage=${paging.cntPerPage }">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.endPage != paging.lastPage }">
			<a href="jp_post-Gallery.past?userId=${User.userID}&nowPage=${paging.endPage + 1}&cntPerPage=${paging.cntPerPage }">&gt;</a>
		</c:if>
	</div>
