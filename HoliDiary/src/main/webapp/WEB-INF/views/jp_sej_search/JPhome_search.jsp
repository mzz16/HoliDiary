...
<body>
	...
	<c:if test="${curPage != 1 }">
		<a href="JPsearch.page.change?p=${curPage - 1 }&search=${search}"><img id="home_search_pageL" src="resources/sej_img/slider/free-icon-left-yellow.png"></a>
	</c:if>
	</div>
	...
	<c:forEach var="c" items="${contents }">
			...
			<a <%-- href="post.detail.go?postNum=${c.postnum } + &userId=${c.postwriter }" --%>onclick="searchPopupJP('${c.postnum }','${c.postwriter }')"
			...
			<span class="home_search_recommend">ライク: ${c.postrecommend }</span>
			</div>
	...
	<c:if test="${curPage != pageCount }">
		<a href="JPsearch.page.change?p=${curPage + 1 }&search=${search}"><img id="home_search_pageR" src="resources/sej_img/slider/free-icon-right-yellow.png"></a>
	</c:if> 
	</div>
	</div>
</body>
