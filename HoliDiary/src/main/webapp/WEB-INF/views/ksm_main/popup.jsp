			...
			<c:choose>
				<c:when
					...
					<button class="btn-invite"
						...
						style="background: ${Diary.themeColor}">다이어리 설정</button>
				</c:when>
				<c:otherwise>
					<button class="btn-invite"
						onclick="location.href='popupHomeGo?userId=${sessionScope.loginUser.userID }'"
						style="background: ${Diary.themeColor}">나의 다이어리로</button>
				</c:otherwise>
			</c:choose>
					...
					<c:when test="${not empty Subcribe}">
						<div class="action-buttons-wrapper">
							<button class="action-buttons btn-upload"
								...
								onclick="subscribeCancel('${Diary.diaryUserId}');">구독취소</button>
						</div>
					</c:when>
					<c:otherwise>
						<div class="action-buttons-wrapper">
							<button class="action-buttons btn-upload"
								style="background: ${Diary.themeColor}"
								onclick="subscribeDo('${Diary.diaryUserId}');">구독</button>
						</div>
					</c:otherwise>
				</c:choose>
